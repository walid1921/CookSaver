...
const AddRecipe = ({ addRecipe, setAddRecipe }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const ingredients = [];
    for (let i = 1; i <= 6; i++) {
      const quantity = formData[`ingredient${i}`]?.split(',')[0];
      const unit = formData[`ingredient${i}`]?.split(',')[1];
      const description = formData[`ingredient${i}`]?.split(',')[2];
  ...
  return (
    <div className='w-full h-full fixed top-0 right-0 z-20 backdrop-blur-sm bg-black/70'>
      ...
      <form className='w-[300px] sm:w-[400px] md:w-[800px] absolute top-1/2 left-1/2 bg-white z-20 rounded-lg transform -translate-x-1/2 -translate-y-1/2 p-5 ' onSubmit={handleSubmit}>
